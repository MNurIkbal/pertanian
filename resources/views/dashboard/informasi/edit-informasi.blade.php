        ...
        <div class="col-lg-12 mb-4 order-0">
            <div class="card">
                <div class="d-flex align-items-end row">
                    ...
                    <div class="card-body">
                        <form action="{{ route('informasi.update', ['id' => Crypt::encrypt($informasi->id)]) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="judul">Judul</label>
                                <input type="text" id="judul" name="judul" class="form-control @error('judul') is-invalid
                            @enderror" placeholder="Judul anda" value="{{ old('judul', $informasi->judul) }}" required>
                                @error('judul')
                            ...
                            @enderror" id="isi-body" name="body" rows="4"
                                    required>{!! old('body', $informasi->body) !!}</textarea>
                                @error('body')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
